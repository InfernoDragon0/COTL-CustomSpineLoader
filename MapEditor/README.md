...
**Structure quick pick dock.** A nine-slot bar above the status bar holding the last nine things
placed this session, newest first — a hotbar. Click a slot or press its number to arm that thing
again; the slot with the currently armed pick wears a ring, and a slot under the pointer lights up
and names what it holds in the status bar. A slot whose icon has not arrived, or which has none,
shows the thing's name instead of sitting blank, the way a browser cell does. It is **on** by
