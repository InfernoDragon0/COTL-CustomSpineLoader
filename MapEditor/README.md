...
**Three plates, two edges** (`MapEditor/Chrome/`). The shell was seven panels on three edges, three
of them stacked in the bottom centre over the room being edited. It is now a top bar, a right
sidebar and a bottom bar, built by `EditorTopBar`, `EditorSidebar` and `EditorBottomBar` on the
plate factory in `EditorChrome`. The classes take an `IMapEditorHost`, a parent rect and a
*blocker delegate* rather than a `RuntimeMapEditor`, so the full-screen screens can reuse them and
register their blockers on their own list.

**Top bar.** The map name in the heading font, a context badge (Dungeon room / Hub / Base), an amber
dot while there is unsaved work, then Load, Save, Clear and — in a dungeon room only — the Level and
Dungeon Builder screens, then `Ctrl+Z` and the `F6` / `F5` / `F4` keys as clickable chips. Clear,
Level and Dungeon Builder answer with a dropdown at the button rather than a sidebar panel, since
each of those panels existed only to ask which. The title label *is*
the one the inline rename prompt writes into, which is why the bar builds it once and only ever
rebuilds the action cluster: the Lapture heading font arrives asynchronously, and a rebuilt label
would drop the pending swap. The name is measured and clamped rather than left to grow, so a long
map name ellipses instead of pushing the buttons off the bar. The far right is left empty for the
version watermark.

Load, Level and Dungeon Builder live here and not in the dock because each takes the whole screen
the moment it is chosen — they are screens, not tools. `DockTools()` filters them out, which also
stops the wheel cycling onto a full-screen editor. Clear is filtered out for the opposite reason: it
is a menu of one-shot actions, never a mode the mapper is *in*, and a dock slot that can never be the
active tool reads as a broken one. Picking from Level's or Dungeon's list makes that tool active
until the screen closes, and closing returns to the tool that was in hand before
(`SelectPreviousTool`); picking from Clear's leaves the active tool alone, since there is no screen
to come back from.

**Sidebar** (right, 420 wide). The active tool's options on top, the layer tree below, one foldable
header each (`MapEditorUI.CreateSection`). The column is a fixed height and the two share it:
whichever the mapper opened last gets what it asks for, down to a floor of 40 per cent for the
other, and the loser scrolls. A tool whose panel is taller than the whole column — Structures with
its 620px grid, Lighting with its sliders — folds the tree away by itself on the tool change, once,
and stops doing that as soon as the mapper opens the tree by hand. Width stays 420 so
`MapEditorUI.IconPreviewRightOffset` still puts the hover preview clear of it.

**Bottom bar.** The tool dock on the left at 60px icons, then the status message, then the active
tool's shortcuts as key chips. The bar carries **no layout group of its own**: the dock sizes itself
to its icons with a `ContentSizeFitter`, and a fitter under a parent layout group is the one
combination Unity will not resolve, so `LayoutAfterDock()` measures the filled dock and moves the
status region to start after it. Chips are measured (`MapEditorKeyChip`) and the ones that would not
fit are hidden rather than clipped mid-word; the row is re-fitted every frame because the text
scaler settles a frame or two late and the canvas is matched half on width, so the row is a
different number of pixels wide on every aspect ratio.

**Shortcut card** (`ShortcutCard`, `?` chip or `F1`). The full list over the room. Everything true
under *every* tool — pan, zoom, wheel, undo, quicksave, the function keys — lives only here, which
is what let each tool's hints shrink to one row of chips; the tool lists themselves were shortened
to a verb or two at the same time. The multiplayer "Resync with <peer>" and the NetVerbose "Sync
self-test" buttons moved here from the old shortcut column. It is built at startup and left
inactive, not created on first use, because `SetOwnChromeVisible` hides the editor by walking the
canvas children captured in `CreateUi` — anything made later would still draw over the Load screen.

**Status.** Still the editor's only feedback channel, but the pulse now tints only the status region
rather than the whole bar, and that region is a plain plate rather than a dressed one: `Dress`
re-applies its tint asynchronously through `Redress`, which used to overwrite the urgent tint the
moment the panel art finished loading.

**Layer tree in the sidebar.** It carries no plate and no header of its own any more — the sidebar
section owns both — and it gained a filter row (`MapEditorSearchRow`, "Filter layers..."). A filter
narrows the rows, not the room: a section whose *name* matches keeps all its items, a section
holding a match is forced open so the match is not buried in a fold, and the header summary reads
"n of m" while filtering. Counts are stale while the tree is folded because `Tick` does not scan
then; one scan runs on the first open.

**Quick pick** docks flush onto the bottom bar's left edge rather than floating above it. It stays
visible under every tool, as before: picking a slot brings the Structures tool forward through
`ActivateTool`.
...
**Toggles stayed rows; chips were rejected.** Squeezing the Structures tool's six placement toggles
into a wrapping row of chips would have bought back about 100px of grid height, and only there —
every one of the fifteen tools uses `CreateToggle`, so the saving would have come at the price of
two different ways to draw the same control. They are grouped under a foldable "Placement" header
instead, which gets the height back when it is closed and keeps one toggle look everywhere. The
Structures category picker stayed a dropdown for the same reason a chip row was wrong for it: the
game's prop groups change between updates, and a dropdown is one row tall whatever the count.

**An ellipsizing TMP label reports its *truncated* width.** This cost three attempts at the top bar's
title, all of them variations on measuring more carefully. Measuring once in `SetTitle` was wrong
because the Lapture heading font arrives asynchronously and `MMTextScaler` resizes the label again
after that. Re-measuring every frame from `preferredWidth` was wrong for a subtler reason: while the
label is set to `TextOverflowModes.Ellipsis`, `preferredWidth` answers with the *cut* string, so once
the title had been shortened it could never ask for the room to grow back and the ellipsis was
permanent. There is no measurement that fixes this — **the label must not truncate at all**. The
title runs `Overflow` and its `LayoutElement` width is re-read from it every frame, clamped at
620px, which is right whatever moves underneath it. The badge is measured the same way.
...
**Boss & special room pieces** is a group of its own, and everything in it is scenery that is *not*
a prefab of its own: the chains hanging in the Death Cat room, the dressing in the boss and miniboss
rooms, the Death Room's plots. Those pieces exist only as children inside a room prefab, so the
group opens each room the way the Enemy tool opens them for its bosses and lists every drawn node it
finds. Picking one copies that node out of the prefab and into the room. The scan runs the first
time the group is opened and is kept for the session; the search field finds those pieces once it
has run, and not before, because scanning every boss room to answer a keystroke would stall it.

**Randomised sets break apart.** Some prefabs are bags of options rather than objects — a
`Chains Randomiser`, the `DecsDeath` plots — and the game picks from them on load, which is right
for a generated dungeon and wrong for a room somebody authored: the room looked different every time
it opened, and the pieces could not be selected, moved or deleted because they were children of the
placed bag. Placing one now drops its contents as separate props instead, each one selectable,
movable, deletable and saved individually. **Break apart randomised sets** turns that off; what is
placed then is the whole set with its dice thrown away, which is stable but still one object.

**Structure quick pick dock.** A nine-slot bar sitting flush on the bottom bar's left edge, holding
the last nine things placed this session, newest first — a hotbar. Click a slot or press its number
to arm that thing again; the slot with the currently armed pick wears a ring, and a slot under the
pointer lights up and names what it holds in the status line. A slot whose icon has not arrived, or
which has none, shows the thing's **initials** rather than sitting blank or wearing a clipped name —
two letters read at slot size where six clipped ones do not. Prop icons arrive through the throttled
async loader, so the tool caches the answer and refreshes the bar when the sprite lands; discarding
that callback is why props used to fall back to text permanently. It is **on** by
default and toggled from the Structure tool's panel; once shown it stays on screen under every tool,
and choosing from it brings the Structure tool forward. Dressing a room means placing the same
handful of things over and over, and the browser makes you find them again each time.

**Pinned favourites.** Right-click a cell in the Structure browser to pin it, and it holds a quick
pick slot for good — it no longer matters what you have placed since, or whether you have placed it
at all. A pinned cell wears a gold star in its top-right corner, and so does its slot on the bar.
Right-click again, on the cell or on the slot, to unpin. Favourites fill the bar from slot one in
the order they were pinned and the recently-placed list fills whatever is left, so pinning nine
things turns the bar into a fixed palette. Pins are written to `EditorFavourites.json` beside the
plugin and come back next launch, unlike the recent list, which is the session's. The browser's
group dropdown has a **Favourites** group listing everything pinned, structures and props together.

While the dock is showing, the **mouse wheel walks the bar** instead of cycling tools — up for the
slot before, down for the slot after, wrapping at both ends, and it arms whatever it lands on. Hide
the dock and the wheel goes back to cycling tools. So does a wheel turn while the dock is up but
empty, so the wheel is never a key that does nothing.

**Notes**

- **A placed prop that carries a `Structure` needs a brain or it needs silencing.** `Structure.Structure_Info` is literally `Brain?.Data`, so every `Interaction` on such a prefab reads through a brain the prop path never creates — and `Interaction_Poop.OnInteract` is the proof: its `Activating` getter guards a null brain, its setter does not, so cleaning editor-placed poop threw an NRE per press with no CultTweaker frame in the stack. `PropBrains.Adopt` runs on every placed prop and takes `Structure.Type` (serialised on the prefab, unlike `Structure_Info`) to `BaseDelta.GiveBrain`, which makes a working brain in the base and is a no-op anywhere else; whatever still has no brain afterwards has its interactions disabled, because a prompt that cannot work is worse than no prompt. The hook sits in `BlueprintLoader.ApplyPropTransform` — the one place the loader, the base delta and the network entry kinds all pass through — plus the three interactive spawn paths, break-apart pieces included.
...
**Trigger method** says what sets a volume off, and the three answers are exclusive because they are
answers to the same question. *On player step* is the original behaviour and the default. *Blocking
volume* is the invisible wall the checkbox used to be — a wall the player cannot walk into is a
trigger that cannot fire, so it is a method rather than a flag laid over one. *Enemy health
threshold* watches one enemy and fires when its health reaches 75 / 50 / 25 / 0%, which is the
cutscene-on-the-boss-half-dead case the tool had no way to author. Picking that method reveals two
rows of its own — the enemy to watch (clicked in the world) and the mark to fire at — and hides them
again for the other two, so a volume never shows a setting that will not be read.
...
go off again without clicking through them one at a time. A method that is not *on player step* wins
over both, because it is the larger fact about the volume: **red** is a wall, **pink** is watching an
enemy's health. The distinction shows only while a volume
...
*Clear All Triggers* — now in the **Clear** tool — **asks first.** Choosing it from Clear's menu
raises a *Confirm* / *Cancel* strip along the bottom naming what will go and how many, and nothing
happens until it is answered. It used to arm instead: the button renamed itself to `Delete all N?
Click again` and a second press within four seconds did the wipe, which made a destructive action two
clicks in the same spot. Every sweep in the Clear menu asks the same way now, which also settles an
old inconsistency — this was the only armed button on a panel whose neighbours were just as
destructive and had never asked at all.
...
**It lives in the top bar, not the dock.** Its button drops a menu of its actions, each carrying the
count it would remove, and the one that is chosen asks the question along the bottom as a *Confirm* /
*Cancel* strip — the same shape as the unsaved-work question. It used to be a sidebar panel whose
buttons armed themselves on the first press and renamed themselves to *Click again*, which meant a
destructive action was two clicks in the same place and the safeguard was a label nobody read twice.
It is not a mode either: nothing about the room changes while Clear is "selected", so it never
becomes the active tool and the dock never offers it (`DockTools()` filters it out alongside the
screens).
...
**Level lives in the top bar.** Its button drops the list of saved levels at the button, with *New
level blueprint* as the first entry, and picking one opens the layout screen directly. There is no
sidebar panel: the panel it used to build existed only to ask which level, and it asked in the far
corner of the screen for a button pressed at the top of it. Closing the screen returns to the tool
...
The dungeon builder's arrangement one level down — full screen, wearing the editor's own three
plates: the level's name and an `Esc` chip along the top, the room list down the right, the screen's
own tools and its status along the bottom. Choosing a level from the top bar's Level button goes
straight to this screen, so there is no second set of controls and no half-open state to be in;
closing it returns to whichever tool was in hand before.
...
The Dungeon Builder button in the top bar asks one question — which dungeon — as a dropdown at the
button, with *New dungeon* as its first entry; picking one goes straight to the map screen
(`DungeonMapCanvas`). Closing the screen closes the dungeon and returns to the tool that was in hand
before, so there is no half-open state and no *Edit Map* / *Close Dungeon* pair to keep in step. The
tool builds no sidebar panel at all: the panel existed only to ask which, and the dropdown asks it
where the pointer already is. Closing with edits that were never written says so in the status line.
The screen is the world map's screen with dungeon nodes on it:
...
- **The room editor stands down while it is up.** This is the change that made it stop reading as a
  ...
  host hides its own three plates and everything on them
  ...
  time `CreateUi` finishes is the host's own; anything added later belongs to a tool. Hiding records
  which of those were actually on screen and shows only those back, or the confirm strip and the
  shortcut card would be raised along with the rest.
- **The map is the screen and the chrome sits on its edges.** Content stretches the whole canvas, and
  the screen wears the same three plates the room editor does, built from the same classes: the
  dungeon's name and an `Esc` chip along the top, the node list down the right, and along the bottom
  its own tools (*Preview*, *Save*, *Enter* — there is no *Done*, because `Esc` already closes) with
  the status beside them. The two map screens are one screen with different content on it because
  they are now literally the same code.
- **One way out.** `Esc`, the top bar's `Esc` chip and `F4` all go through `RequestClose`: it
  dismisses the confirm strip if one is up, otherwise asks about unsaved work, otherwise closes. The
  prompt is the shared `MapEditorConfirm` - *Save & close* / *Discard* / *Cancel*, sitting clear of
  the bottom bar - and while it is up the map ignores every click and key but `Esc`. `F4` is
...
- **The verdict and the commentary are separate.** The screen used to draw two stacked lines of its
  own; the standing verdict now sits as a muted note beside the dungeon's name in the top bar, and
  the bottom bar's status line carries what just happened. Side by side in one bar they fought for
  the same middle, and a long message under a long problem simply drew over it.
...
- Closing while the map differs from what is on disk asks first: **Save & close**, **Discard**, or
  **Cancel**. The check is a comparison against the last saved copy rather than a flag raised by
  each widget, so no slider can quietly slip through it. The prompt strip stands clear of the bottom
  bar — it shares the canvas with the editor's chrome, and one drawn behind it reads as nothing
  having happened.
...
The editor wears the same three plates the room editor does, from the same classes (separate
instances; the two editors can never be open at once), with the same accent border around the
selected tool. The **top bar belongs to the map screen rather than the editor**: the map's name sits
there whether it is being played or edited, so leaving edit mode with `F6` takes the tools away and
leaves the map with its name and its way out still on it. Edit mode only changes what the bar's
right-hand end carries — Undo, Save and Play view — and lights an *editing* badge beside the name.
There is no Play button in the tools: **F6** leaves edit mode.
...
The active tool's keys are **chips along the bottom bar**, built from the same `IMapEditorShortcuts`
list the room editor uses; the keys that mean the same under every tool — Ctrl+Z, Ctrl+S, F6, Esc —
are on the **shortcut card** instead, opened by the `?` chip or `F1`. Esc has no row of its own: it
does what F6 does and then closes. The **status line** shares the bottom bar with the chips, saying
what the cursor is over — the dock tool, a widget, a layer row, or the node under the pointer with
its type, unlock state, destination and links — and carrying the editor's own messages when nothing
is hovered. Tool icons come from
...
Other mods drive all of this through the world map members of `CultTweakerApi` (contract 3), by map
name and node id. Three things about that surface came out of writing it, and matter to anyone
touching `WorldMapProgress` again:

- **There can be no "unlock node X" call**, because state is not stored. `WorldMapStateResolver.Resolve`
  recomputes every node from the graph and the record on each refresh, and `Upgrade` only ever
  raises a state, so the API exposes the record - complete, un-complete, open, close, keys - and
  lets state follow. An API shaped like lock/unlock would have been a lie about the model.
- **`For()` hands out the live record while `Save()` is private**, which meant a caller could write
  `For("x").KeysHeld = 3` and watch it vanish at the next slot change. `SetKeys` exists so that is
  not the only way; the same trap is still open for direct list edits, so prefer adding a method to
  reaching into the record.
- **`OpenLock` used to trust its caller on affordability** - the screen checked `KeysHeld >= KeysCost`
  before calling and `OpenLock` only clamped with `Math.Max`. Fine with one caller, wrong the moment
  there were two, so the guard moved into `OpenLock` and it returns bool. `CloseLock` is its exact
  inverse and refunds; `Uncomplete` deliberately is NOT an exact inverse of `MarkCompleted` (it
  un-banks a key node's grant floored at zero, and leaves any lock those keys opened open) because
  shutting a paid-for door reaches beyond the node that was named.

Forcing the record out of order cannot strand a map: a node completed ahead of its prerequisites
just cascades forward from itself, and un-completing removes a cascade source so its children fall
back to `InitialState`. What a mod CAN produce is a map that looks impossible - a shut lock with
completed nodes past it - which the resolver renders without complaint.
...
| `WorldMap/WorldMapEditor.cs`   | The editor host: bottom bar, sidebar, undo, tool plumbing       |
...
- **Drawing where it was authored (DungeonMapViewPatches.cs).** The game's map screen ignores the layout it is handed: MakeMapNode places each node at `point*300 + Random.insideUnitCircle*50` and OnShowStarted rebuilds every node each time the screen opens, so one map drew differently every time it was shown — and the jitter was the smaller half of it, because `point` is the *derived* grid: rows evenly spaced whatever the author did, a short row pushed left instead of sitting under its parent, every layer one step apart. Build writes the editor's own coordinates into `Node.position` (a field the game's generator fills in too and its map screen never reads — `Map.DistanceBetweenFirstAndLastLayers` is its only consumer, itself uncalled), scaled by `ViewScale` = 1/DungeonMapCanvas.NodeScale, since the editor clones the game's node prefab at half size. A postfix on MakeMapNode assigns that position and **everything downstream falls out of it for free** — the bounds the screen centres on, the map height, the scroll offset, the connection lines are all read back off the rects afterwards. Ownership is a ConditionalWeakTable of maps this builder made, so a vanilla run is untouched and a dropped map collects with its entry.
- **The boss rule runs last.** After centring, OnShowStarted drags every `NodeType.Boss` node to x = 0 — too late to head off from MakeMapNode, and it would undo the postfix for exactly those nodes. A second postfix re-applies the whole layout once the screen has finished building; it is a no-op for every node the boss rule did not touch, because the centre is recomputed the way the screen computed it (`Bounds.center` over the same points = their midpoint). Connection lines are rebuilt regardless — `MMUILineRenderer` captures its endpoints when it is made, so moving a node afterwards leaves its lines behind. The crown is re-seated for the same reason.
...
- Input details: leaving a tool (or closing the editor) confirms an open prompt — panels stay live during prompts (that's what lets a search result be clicked), so the dock is reachable mid-word, and walking away used to leave the field wearing a caret. Wheel-over-list check walks ScrollRects back to front (an open dropdown is parented last and must win) and consumes the wheel so it doesn't also switch tools. A missing wheel axis in this build's input manager is remembered (stop asking). Update exceptions are throttled (a broken tool throws every frame). Zoom drives CamFollowTarget.targetDistance (game default 12, not an orthographic size), initialized from wherever the game's zoom is so opening never jumps. MoveCameraTo moves the follow ANCHOR — the rig overwrites the camera transform next frame. ScreenToWorld projects onto z=0 (correct for ortho and perspective). KeepCullingSuspended for tools that move objects out of their culling area. WorldClicksBlocked exists for full-screen surfaces: PointerOverUi is true everywhere inside one (the backdrop is a blocker), so it can't tell widget presses from surface clicks — BlockWorldClicks per widget press is the half that still can. RectangleContainsScreenPoint gets a null camera (screen-space overlay). Status: hover text restores the last real message when cleared, and a hover is passed with `pulse: false` so it repaints the words without clearing the ring on whatever the editor last needed answering; only the plain plate darkens for urgency (on the game's art the accent outline already says it).

### Room editor host, continued (RuntimeMapEditor.cs tail)

- Switching tools closes transient UI (an open dropdown would outlive the previous panel and keep absorbing clicks) and confirms the prompt (keystrokes were being eaten by a search box belonging to the tool just left). CanvasScaler is required (unreadably small at 4K). Attach the UI before building anything (overlays parent to the canvas root; icon fills need a coroutine host). **Own-chrome list is collected at build time** — everything on the canvas then is editor furniture; anything parented later belongs to a tool and must survive the chrome standing down. Everything the editor owns is therefore built BEFORE that snapshot, the confirm strip and the shortcut card included. **The restore records what was actually on screen:** hiding walks the snapshot and remembers only the objects that were active, and showing raises only those. An earlier version raised the whole snapshot, which switched the confirm strip and the shortcut card on wholesale — closing the Load browser asked "save changes?" because restoring the chrome had raised the hidden question. Anything that manages its own visibility depends on this. ConfirmBottom clears the bottom bar — a question behind it reads as nothing happening.
- Dock (now the bottom bar's left end): **detach stale icons before Destroy**. Destroy defers to end of frame, so old icons are still parented and counted when the layout rebuilds — the dock recovered on the next fitter pass but the width read NOW was what positioned the status region beside it. ContentSizeFitter horizontal only (a vertical fit collapses the plate for a frame before icons report sizes); `LayoutAfterDock()` force-rebuilds and then moves the status region, because the bar carries no layout group of its own — a fitter under a controlling parent group is the one combination Unity will not resolve. RefreshDockForContext exists because the dock is built at construction and the context can change under it (the base-scene host is stood up before anyone opens the base editor — its dock was laid out for a dungeon). Options rebuild defers three frames (Destroy end-of-frame + staggered fills still adding cells). Status urgency uses the accent outline, not a washed plate, on the game's art.
...
- **Stranded-door check measures from the FLOOR, not the geometric centre.** `ConnectStrandedDoors` grows a doorway's pad (`ExtendPad`, 8 units a step to 60) until A* finds a path from 3 units inside the door to a reference node. That node used to be `GetNearest(composite.bounds.center)` with no constraint, and a room built around a pit or a tree has its centre IN the hole: the nearest node there is unwalkable, no door could ever reach it, so every pad grew step by step to the far wall, and where a pad crossed the hole it filled it with a 3-unit band of collision (the user's screenshot: the west door's outline running into the central hole). `FloorReferenceNode` asks with `NNConstraint.Default` (walkable nodes only) and falls back to the raw nearest node only if that fails. Each growth step now logs which doorway grew, so a pad longer than 9 is always explained in the log.
...
- **Quick pick is two nested objects, because two different things hide it.** `MapEditorQuickPick` builds an outer node that is a canvas child at `CreateUi` time — which is what puts it in `_ownChrome`, so F6 takes it with the rest of the chrome — and the dressed bar inside it, which is what the tool's toggle shows and hides. One object answering to both would have the two fighting: F6 restoring chrome would re-show a bar the author had turned off. Nine slots are always drawn, empty ones as dark plates, so the geometry never moves as the list fills. The bar records only what a person placed (`Place`, `UpdatePropPlacement`, and the scatter path), never `Finish` — noting from there would fill the bar with the map's own contents every time one was loaded. Entries dedupe to the front rather than appending, or placing the same thing nine times in a row would be the whole bar. Structure icons are a synchronous dictionary lookup but prop icons arrive from a background load, so `Tick` re-reads them twice a second while the bar is up; a slot can be filled a moment before it can be drawn. Choosing a slot routes through `SelectStructure`/`SelectProp` — the same calls a browser cell makes, so scatter mode's multi-select and the "click the armed thing again to disarm" behaviour come along for free instead of being reimplemented.
- **Favourites are keyed by name, not by the enum.** A custom structure's `StructureBrain.TYPES` is minted while the mod loads and differs between sessions, so `EditorFavourites.json` stores `InternalNameOf(type)` and `Resolve` asks `TryResolveAnyType` for the value on read — persisting the enum would have a favourite silently become a different building after a mod was added. A favourite whose mod is gone is SKIPPED, not deleted: reinstalling brings the pin back, and deleting would punish a player for temporarily removing a mod. Grid cell ids are the tool's own (`type:<TYPES>` for structures, the prefab path for props), so `EntryForCell` translates them into the quick pick's keys (`structure:<internal name>` / `prop:<path>`) — the bar and the file must agree on one key or the star and the slot would disagree about what is pinned. The totem cell refuses a pin (one per hub, so a shortcut to it could only ever be refused). The star badge on a browser cell is built the first time that cell needs one — the browser can hold several hundred cells and almost none are favourites — while the bar's nine are made up front and hidden. `Compose` rebuilds the shown list inside `Refresh`, so a pin taken from the bar, from a cell, or from another session's file all land the same way; recents are still capped at nine rather than at the space left over, so unpinning reveals the next thing placed instead of an empty slot. Right-click reaches a browser cell through `MapEditorRightClick`, a second `IPointerClickHandler` alongside the `Button`: Unity's `Button` drops anything that is not the left mouse button, so the two never collide, and the editor's own `StandaloneInputModule` is what dispatches button 1 at all.
- **The wheel is claimed, not stolen.** `HandleWheel` asks the scroll rect under the pointer first (unchanged), then offers the turn to `QuickPick.Step`, and only cycles tools if the bar declines — which it does whenever it is hidden or empty, so enabling the dock can never leave the wheel inert. `Step` reads the armed slot back through the same `IsArmed` the ring uses rather than keeping a cursor of its own: a pick armed from a browser cell, a number key or the bar itself all leave the wheel stepping from the right place, and a cursor would have drifted out of step with the ring the moment anything else armed something. Wrapping is `((i + d) % n + n) % n` because C#'s `%` keeps the sign of the left operand, so a step back from slot one would otherwise land on -1. Both the wheel and the number keys pass `blockWorldClicks: false` — the 0.2s block exists so the press that chose a slot cannot also place something, and a keyboard or wheel gesture that swallowed the next click would be a worse bug than the one it guards against. Stepping arms through `OnPick` like a click, so it also brings the Structure tool forward from whatever tool is active; that is deliberate (an armed structure means nothing under the Trigger tool) but it does mean the wheel changes tools once, in one direction, on the first turn.
- **Slot hover lights an overlay, not the plate.** The tool dock can let `MapEditorHover` tint its plate because that colour never changes; a quick pick slot's plate is owned by `Refresh` (empty slots are darker than filled ones) and the two would overwrite each other frame by frame. So each slot carries a transparent child image that hover raises instead. The status text comes from `HoverTextProvider`, not `HoverText`, because what a slot holds changes under a fixed component — and it names the gesture ("pinned, right-click to unpin"), which is the only place that pairing is ever stated. `RuntimeMapEditor.CreateUi` shows the bar AFTER the tool loop, not in the quick pick's constructor: `StructureTool.BuildPanel` is what hands it `Icon` and `Favourites`, so showing it any earlier would draw nine blank slots and leave them blank until the first tick.
- **The Favourites group carries no count in its label.** Dropdown options are fixed when the panel is built, so `Favourites (3)` would be wrong the first time anything was pinned. It sits at index 1, not 0, so it never becomes the group the browser opens on — an empty Favourites as the default view would look like a broken browser on a fresh install. Toggling a pin while that group is showing re-populates it rather than only re-starring cells, because there the favourites ARE the cells.
- **Structures load as a batch, not one at a time (`PlaceMany`).** Addressables never completes an `InstantiateAsync` in the frame it was asked for — even for an asset already in memory, the ResourceManager finishes it on its next update — so a loop of `yield return PlaceAt(...)` costs at least one frame per structure before the next one starts. That, not the weight of what is being built, is why a map with many custom structures took a visible while to open: every custom structure comes from the same small COTL_API base prefab, `GiveBrain` no-ops outside the base, and the donor-material sweeps in `SeeThrough`/`StructureShadows` are behind one-shot latches, leaving `SkeletonAnimation.Initialize` as the only real per-instance cost and that is milliseconds. `PlaceAt` was split into `Begin` (resolve the path, start the handle) / `Finish` (everything after it lands) / `Relook`, so the batch starts every load, waits once for all of them, then settles them in order. `Finish` runs `quiet` in a batch — one status line per structure would be a flicker, not a readout. Both the blueprint loader and the base delta hand it their whole list and take the instance back through a callback, which also retires the `LastPlacedInstance` before/after dance those loops used to identify what they had just placed.
- **DEAD END: skipping the one-frame re-dress when no brain was added.** `GiveBrain` returns without doing anything outside the base, so the `yield return null` before the second `SeeThrough.Set` looks like pure waste in a dungeon. It is not: a `Structure.Start` postfix swaps the art on overridden vanilla buildings, and that lands a frame late in a dungeon as much as in the base, so the second dress has to happen either way. Batching makes it moot — the whole load now waits one frame for every re-dress together instead of one frame each.
- **Randomisers are stripped from everything the editor spawns (`PropRandomisers.Freeze`).** `randomChildPicker`, `RandomChildPickerWeighted`, `RandomInstantiator`, `RandomObjectPicker`, `RandomBushPicker` and `RandomGrassPicker` decide which objects exist; `RandomEnable`, `RandomSpritePicker` and the spine/frame pickers vary one object's look. Both kinds go. The timing is the whole trick: the pickers roll in `Start`, and a spawn callback runs in the same frame the object was instantiated — before the first Update, so a picker destroyed there never rolls at all and the prefab stays exactly as it was authored. `DestroyImmediate`, not `Destroy`, because a deferred destroy would still be pending when Start came round. The exception is `RandomEnable`, which rolls in `OnEnable` during `Instantiate` itself — too early to beat; freezing keeps whatever it rolled, and freezing BEFORE `SetActive(true)` is what stops it switching a lifted piece straight back off.
- **A randomised set is broken into its pieces at place time**, which is the only point where the roll can be resolved once and written down. `BreakApart` reads each piece's path inside the prefab *before* reparenting (the path is the thing that finds it again), lifts the children of every structural randomiser into the room's scenery with `SetParent(worldPositionStays: true)`, then destroys the husk. Recording a "which children were on" mask on the placed bag was the other option and is strictly worse: it fixes the flicker but leaves every piece unselectable, which was half the complaint. Order matters inside the callback — break apart first, freeze second, because freezing destroys the very components that say where the pieces are.
- **`CTPropSource` is how a lifted piece gets saved.** The room snapshot resolves props through `ObjectPool.spawnedObjects` or by matching the name against the catalog, and a child pulled out of another prefab answers to neither — the pool never saw it and its name is not a catalog name. So the object carries its own key (`room:<owning prefab>|<path inside it>`, the `RoomChildPrefabs` form the Enemy tool already uses for boss-room characters) and `TryResolve` checks that first. `BlueprintLoader.SpawnProps` and the multiplayer `PropKind` both grow a matching branch: load the owning prefab, walk to the path, copy it out. The sweep's recursion into a resolved prop then skips the piece's own children as usual, so nothing is written twice.
...
- **Blocking volume = an invisible wall, and it is built by NOT joining anything.** A child "Blocker" carries a solid BoxCollider2D on the Obstacles layer, sized with the volume. Standing alone — no composite, no rigidbody — a collider is a static solid that pushes whatever touches it (the trick DoorTool's plugs use to seal a doorway); merging it into the room composite would do the exact opposite, since that geometry is Outlines and anything joined to it becomes walkable floor. Obstacles is the layer the player, the enemies and the grid graph all read as terrain, so one box stops both parties — navigation is rescanned when the method changes into or out of Blocking and when a blocking volume finishes a move/resize, never per drag frame. Red outline, and blocking beats the spent/fired greys (a wall that happens to have fired is still a wall) while selection yellow and the firing flash still win. The collider is live while editing too: the clock is stopped, but a wall drawn over where the players stand will shove them when play resumes.
- **Trigger method (`TriggerMethod`, `MapTriggerData.Method`) replaced the Blocking checkbox.** `Blocking` is now a read-only property over the enum, so every existing reader keeps working and only one field can say what a volume is; the bool is still WRITTEN to the file beside the new string, and `MethodOf` prefers the string and falls back to the bool, so a map saved by this build still opens (as a wall, or a plain trigger) in a build that predates it and a map saved before it loads correctly here. The trip collider is switched OFF for the other two methods (`Refresh`) rather than guarded at every reader — a blocking volume never fires because a wall cannot be entered, and a health-watching volume is a place, not a doorway. `CreateTrigger` gained a `MapTriggerData` overload and every loader (blueprint, base delta, the network entry kind) was moved onto it: the argument list was already nine long, and the record is the shape all three of them are holding anyway; the clone path round-trips through it too, so a method or a watched enemy cannot be forgotten in a copy.
- **The health watch polls, and fires on the CROSSING.** `Health` raises nothing on damage that is cheap to subscribe to per-enemy, so `CTMapTrigger.Update` reads `HP / totalHP` five times a second while the method is EnemyHealth — small, and only for volumes that asked. State, not crossing, would fire a repeating trigger on every poll for as long as the enemy stayed hurt; a remembered previous fraction makes healing back above the mark the re-arm, which is what a repeating trigger means. The FIRST look counts as a crossing, so an enemy already below the mark when it is found still sets the volume off. A resolved enemy that later goes missing reads as 0% — it was killed, and the object is destroyed before its HP can be read as zero — but a target that has NEVER resolved is not treated as dead: a room's contents spawn over several frames and "not found yet" would otherwise fire every 0% trigger in the room on load. `Refresh`/`Rearm` drop the cached `Health` and the remembered fraction.
- **Object targets can be an editor id (`#` + id) instead of a scene path.** Two Enemy Dogs in one room have the same name and `GameObject.Find` returns whichever it likes; the ids added for multiplayer already round-trip through save/load for everything the tools place, so `TriggerActions.TargetOf` prefers one and `ResolveObject` reads the prefix. Nothing older writes it, so old files are untouched, and a path is still what an untracked vanilla object gets. `DisplayName` resolves the id back to a name for the panel — an id is a label only while the thing it names is in the room.
- **Pausing enemy AI = disabling the `UnitObject` behaviour** (the whole AI lives in its `Update`/`FixedUpdate` override — there is no pause flag on it and no game-wide freeze that is not `Time.timeScale`), with `speed`/`moveVX`/`moveVY`/`pathToFollow` cleared so `FixedUpdate` cannot carry the last frame's velocity into the resume, and every child `SkeletonAnimation` set to `timeScale = 0`. The spine has to be frozen too: the animation is set from the enemy's own `Update`, so disabling that alone leaves a walk cycle looping in place. Original time scales are remembered per skeleton rather than assumed to be 1. `Health.team2` is the enumeration for "every enemy" (the game's own register, so enemies that spawn later are included; entries with no `UnitObject` are breakable scenery and are skipped). `ResumeAll` hangs off `ResetSequenceState`, which already runs on room teardown — a paused enemy handed to the next room would be a permanently dead one.
...
- **Drag nodes are one ring sprite, tinted (`MapEditorUI.HandleRing` / `DressHandle`).** Every node in the editor — the Select tool's yellow grip, blue resize and violet depth, the Trigger tool's move and resize, the Shape tool's centre and points, the Door tool's dots, the world map's corners — used to be a solid square or plate, which covered the thing being dragged at the moment of dragging it. They are now hollow: a hard ring with the colour's own fill at 0.28 alpha inside. The sprite is drawn white throughout with the SHAPE carried entirely in the alpha channel, so a single `Image.color` tints ring and fill together and one sprite serves every node — a per-colour sprite would be six textures that could drift apart. Simple, never sliced (a circle has no nine-part stretch), and both edges of the ring feather over one pixel with the disc taking what is left inside, so the two do not double up into a bright seam. `HandleGrowth` (1.35) is applied inside `DressHandle` rather than at the call sites, so the old sizes stay readable as the sizes they were: an outline is a smaller target than a filled shape of the same width, and the nodes needed the width back. The hollow centre still drags — `Image` raycasts its whole rect unless `alphaHitTestMinimumThreshold` is set, which it is not.
...
- WorldMapEditor host: selection by ID, not reference (rebuilds replace the objects underneath). Tool order: nodes first and selected first (the map is made of nodes; layers dress it, file is housekeeping). Attach(this), not null — passing null dropped hover lines and blocker rects on the floor. The hovered thing keeps the bar until the cursor leaves; map hover is POLLED (nodes are picked geometrically, not through the event system) and skipped over panels (widgets report themselves — stepping in would wipe what they said); re-shown when the bar fell back to the status message (leaving a widget with the cursor still on a node reads the node again). Quicksave arming as the room editor (NoteSaved from the File tool's dialog). Update errors throttled per interval. **The chrome is the shared one** (`MapEditor/Chrome/`), but split between two owners: the top bar belongs to the SCREEN, not the editor, because the screen outlives edit mode — `F6` leaves the map on screen with its name and its way out, and a bar the editor owned would go with it. So `WorldMapScreen` builds the bar and carries the title, and the editor only swaps what is on its right (Undo/Save/Play view while editing, Esc to close otherwise) and lights the badge. Everything else — bottom bar, sidebar, shortcut card — is built per `EnterEditMode` and destroyed on exit, and `SetPlayActions` runs on exit whether or not the screen is still open, or a map closed mid-edit would come back up wearing the editor's keys. The unsaved dot is POLLED once a second rather than per frame: asking means serialising the whole map. Escape is read by the screen alone (two MonoBehaviours polling the same key in undefined order), so `StepBack` asks the editor whether the shortcut card is what should close first. Blockers deduped AND pruned walking backwards (dropdown floats register per open, destroyed on close — an append-only list gains a dead entry per open). BlockWorldClicks is a 0.2s window (a widget click and the tools' polling see the same frame; a dropdown list stands outside every registered rect). RequestOptionsResize = 3 frames (Destroy end-of-frame + staggered fills). The selection ring toggles the GAMEOBJECT, not the Image (CreateIconButton's border starts switched off — enabling the component alone left it invisible).
...
- TakeInput: UINavigatorNew LockInput/LockNavigation (the game's own idiom — the intro uses it), CanvasGroup.blocksRaycasts=false on the menu (a click that misses a panel otherwise lands on Quit), cursor forced visible (the most gamepad-likely screen hides the cursor on seeing a controller; an editor nobody can point at is not an editor). ModalOpen lifts the navigator lock while the name dialog is up (it is one of the game's own menus, driven by the very navigator we locked — holding it gives a text box you cannot type into) and re-takes it after. RegisterUiBlocker/BlockWorldClicks are deliberate no-ops — nothing behind the panels is clickable, so the registered rects have no consumer, and the chrome is handed a null blocker delegate rather than one that discards. The menu editor wears the shared chrome outright (top bar, bottom bar, sidebar, shortcut card, all on its own canvas, all destroyed with it) — it had a near-copy of the room editor's dock, options panel, status strip and shortcut column, which is how the two drifted apart on padding and width. Because it lives outside `CustomSpineLoader.MapEditor`, it needs `using Chrome = CustomSpineLoader.MapEditor.Chrome;`: a using-namespace directive imports a namespace's TYPES, never its nested namespaces.
...
### Patches (AnimatorRebindPatches.cs, BaseEditPatches.cs, CustomEnemyPatches.cs, DungeonMapViewPatches.cs, DungeonPatches.cs, HubBuildPatches.cs, MainMenuPatches.cs, SkeletonAssetPatches.cs)
...
- DungeonMapViewPatches: two postfixes on UIAdventureMapOverlayController that draw a custom dungeon map where its editor drew it, instead of on the derived grid plus per-open random jitter. Gated on DungeonMapBuilder.IsAuthored, so a generated map keeps vanilla's placement exactly. Covered in full under "Dungeon maps".
...
- **The stripped follower rig (`followerSkel/stripped/`, made by `tools/spine38.py`).** The game's `Follower.skel` is a 20 MB Spine **3.8.99 binary** — 624 skins, 1036 animations, 86k-entry string table — and takes minutes to open in Spine, which is what stopped anyone authoring follower clothes. Deleting skins inside the editor "made the skin uneven" because in 3.8 a skin OWNS bones and constraints (every animal owns `EYE_LEFT`/`EYE_RIGHT`/`MOUTH`; `Mutation/*` own four) and the editor deletes those with the skin, so the survivors' meshes lose their weights. The tool is a Python port of the 3.8 `SkeletonBinary` reader plus a 3.8 JSON writer; it keeps every bone, slot and constraint, keeps `default` plus the named skins and animations, and drops the rest. Things that bit while writing it, kept here so the next reader does not re-derive them: a named skin's name is a **string REF** (index into the table), not an inline string, unlike bone/slot/constraint/animation names — the first parse desynced on exactly that byte; `hull` is in the same units in binary and JSON (vertex count; both readers double it); a **linked mesh names its parent by skin**, so a kept skin (`default` included — it has its own) whose parent skin is dropped must have the parent's mesh data inlined, 23 of them for the Cat cut, and `skin=null` on a linked mesh means the default skin, which is never dropped; deform timelines are keyed by skin and must be filtered to the kept ones. The game export was made with `nonessential=false`, so the binary carries **no mesh `edges` and no `width`/`height`** — without edges Spine re-triangulates from the hull on import and the game's triangulation is lost, so the writer emits every triangle edge (as vertex index ×2, the editor's encoding, confirmed against a real 3.8 export) as an authored edge, and takes sizes from the atlas region's `orig`. The default-skin round trip is checked by `validate` (every reference resolves, every index in range) and by comparing key shapes against an existing 3.8.99 JSON the loader already accepts. Images: the atlas is unpacked per region (`rotate` honoured, `orig`/`offset` restored) into `images/` so the editor can import the JSON straight away — 363 files for the Cat cut versus one 8192² page. Windows Python cannot see git-bash's `/tmp`; scratch output goes to the scratchpad path.
- **The face slid off Cat in the first cut, and skin-OWNED bones were not the cause — orphaned skin bones were.** Cat owns no constraints at all, and every constraint on the face bones is owned by some other animal — except `ADJUST_DRAGONFLY_EYE1/2`, which are NOT skin-required (always on), pull `EYE_LEFT`/`EYE_RIGHT` toward `ADJUST_HAMMERHEAD` with offsets of +578/−758 at `translateMix` 0.04 (≈ +23/−30 px), and `ADJUST_HAMMERHEAD` is a skin bone only the Hammerhead skins own. The 3.8 runtime's `UpdateCache` makes a constraint inactive whenever its TARGET bone is inactive, and a skin bone is inactive under any skin that does not own it — so in the game those two never run for a Cat. The editor mirrors that only while some skin owns the bone: once none does (skins deleted by hand, or stripped), it treats the bone as ordinary, the constraints wake, and the eyes move — the same failure by the same mechanism both times, which is why keeping every constraint "untouched" was the wrong fix. `retire_unowned` now drops (a) skin-required constraints no kept skin owns and (b) ANY constraint whose target or constrained bones include a skin bone no kept skin owns (ancestors of owned skin bones count as owned, as the runtime activates them), clears `skin` on the orphaned bones so the file is consistent as the editor reads it, and filters the animations' constraint timelines to match; `validate` now checks those timelines. For the Cat cut that is 38 of 48 constraints and 11 of 14 skin bones, and no kept mesh is weighted to any of them — bone indices are untouched, so weights need no remap. The cost is real and documented in the author notes: a cut has only the constraints its skins use, so a hood (`HoodedFaceLock`) needs a cut that includes a Hooded skin. **The editor's importer requires constraint `order` to run 0..N-1 across IK, transform and path together** (`Missing constraint order: 4`); the runtime does not care, which is why `validate` had to grow the check — retiring constraints leaves gaps, so survivors are renumbered in the relative order they had.

- **A part whose attachment name has a slash never loaded: `Material with texture name "MOUTH_SAD" not found for atlas asset`.** `AtlasFor` named the part's texture and its one-page atlas `<skin>_<part>` — `Aniel_base_Face/MOUTH_SAD` — and spine-unity's `MaterialsTextureLoader` matches a page to a material by `Path.GetFileNameWithoutExtension(page)`, so it looked for a texture called `MOUTH_SAD` and found `Aniel_base_Face/MOUTH_SAD`. Every Cat mouth is `Face/MOUTH_*`, so mouths were un-overridable until 2026-09-14; `PageName` now flattens `/` and `\` to `_` for the texture, material and page name (the region name inside the atlas keeps the slash, which is fine — only the page goes through the path helper).
- **The same attachment name in two slots crashed the bake: `IndexOutOfRangeException` in `COTL_API.CustomSkins.CustomSkinManager.Graphics_CopyTexture`.** COTL_API prefixes `Graphics.CopyTexture` to convert a source whose format differs from the destination (its reason is the BC7 page) and caches the converted copy in a dictionary keyed by `Texture.name`. Our part textures were named `<skin>_<part>`, and EYE_LEFT and EYE_RIGHT both hold attachments called `EYE`, `EYE_CLOSED`, `EYE_SQUINT`..., so the second eye's copy read the FIRST eye's cached pixels with its own width/height — wrong art when the sizes matched, an overrun when they did not. `PageName` now includes the slot index. For packs that must run on older builds the only lever is the config: pad the two crops to one size and give the right eye a negative `scaleX`, since it is drawn from the left eye's pixels anyway (`config.legacy.json` in the Aniel folder does this; the trick is wrong on the fixed build).
- **Authoring a whole character with the FollowerSkins method (the Aniel test follower, `followerSkel/LambWar/claudeFollower`, 2026-09-14).** Three findings that make it possible without touching the loader. (1) The Cat skin carries four invisible placeholder slots on the HEAD bone — `EXTRA_BTM` (88, behind the face), `MARKINGS` (92), `EXTRA_TOP` (94, before the mouth and eyes, with an `EXTRA_TOP_BACK` for facing away) and `LESHY_FACE` (103, over the eyes) — each a `Face/EYE` region at scale 0.001; other animals put horns (Lamb), a horn plus face variants (Unicorn) and markings there, and the `-up` animations key MARKINGS/LESHY_FACE to none. Overriding them is how hair, horns and glasses get their own layers; the config `scaleX/Y` REPLACES the 0.001, it does not multiply. (2) A follower skin cannot supply clothes: `SetFollowerCostume` always adds a `Clothes/*` skin (clothing None → `Clothes/Robes_Lvl1`) and every robe fills BODY_TOP/BODY_BTM/BODY_EXTRA/SHAWL_*/SLEEVE_* with attachments of the SAME names, so a skin's body entries are overridden by `AddSkin`. Clothes go in a FollowerSpines pack; blank 4×4 regions in the sleeve and extra slots hide the game's. (3) The loader's placement maths is fit-to-box: a region override keeps the base attachment's width/height and stretches the png into that box times the config scale; a mesh override becomes a region sized to the mesh's bounding box in the runtime `Vertices` array — for a weighted mesh that array is (x, y, weight) triples in EACH influence bone's local space, and the `j % 3` switch reads x as "Y" and y as "X", which the swapped `X = centerY - offsetY` / `Y = centerX - offsetX` then undoes; `Width = diffX` is really the bone-local y extent, correct for a −90° region. Offsets are in skeleton units, and the follower asset's scale is **0.005** (`WorshipperData.Instance.SkeletonData.skeletonDataAsset.scale`, the value the wardrobe logs): 1.0 = 200 px, so the F8 sliders' −1..1 span ±200 px. The first Aniel cut assumed spine-unity's default 0.01 and every part landed twice as far from its base position as intended. `tools/lambwar_spine.py` (gitignored scratch) re-implements both branches plus a setup-pose renderer (bone world transforms, weighted meshes, region quads) so a pack can be previewed and solved for offsets without the game; `tools/lambwar_build.py` builds Aniel from it. Eye bones are scaled 0.8165, so an eye override's scale must divide by the bone scale to come out 1:1. The robe meshes can be resized for a wider or longer body by scaling every influence's local vertex about a world anchor (`local = bone.local(P + k(bone.world(v) − P))`); with the weights summing to 1 the setup-pose shape scales exactly and animation deforms still apply.

### Follower wardrobe: hats and clothes as overlays (SpineLoaderHelper/FollowerWardrobe.cs)

- **Why an overlay and not the PNG override system.** `ComposeSkin` copies one whole animal skin and swaps PNGs only where the base already has an attachment; `Cat` has nothing on `HAT_NORMAL`/`HAT_UP`/`Necklace`/`SHAWL_*`/`SLEEVE_*_TOP`, so a hat literally cannot be authored there, meshes are flattened to quads, and every variant repacks an atlas at boot. The game already composes a follower by layering whole skins (`FollowerBrain.SetFollowerCostume` static: animal → `Clothes/*` → `Necklaces/*` → outfit/special → hood robes → `Hats/*` → `Necklaces/Scarf`, each `Skin.AddSkin` onto one fresh `new Skin("New Skin")`, later wins per (slotIndex, name)), so a custom hat is just another skin added after the game's. `FollowerWardrobe.Dress` runs from a `SetFollowerCostume` postfix on the composite the game just built (refuses a data skin, as `HideSlots` does): custom clothes first, then the game's necklace/hat/scarf layers re-added on top (their names come from the private `GetHatName`/`GetNecklaceName`/`GetCustomisationName` via `AccessTools.MethodDelegate`, not re-implemented), then the custom hat, then `SetSlotsToSetupPose` so slots resolve the new attachments by their setup names. Nothing is baked; donor attachments go in by reference with their own material, the same cross-atlas path fleeces use.
- **Cross-skeleton rules that the player side never needed.** Slot indexes are per skeleton, so donor entries are mapped by slot NAME (`donorData.Slots[entry.SlotIndex].Name` → `skeleton.FindSlotIndex`). Weighted meshes index the BONE list and are copied by reference, so the donor's bone list must be a prefix of the game's in order (`BonesLineUp`); a longer or reordered one is refused at load rather than deforming or throwing mid-render. That is why `spine38.py strip` keeps every bone. A slot shows only the attachment its setup pose / animations name, so a donor slot holding exactly one attachment under some other name is also set under the slot's `AttachmentName` (the broom `OnlyAttachmentOn` leniency, applied at write rather than read).
- **Persistence and preview.** The choice lives on the follower's `CustomFollowerColor` record (`CustomHat`/`CustomClothes`, keys `<pack>/<skin>`) in `CustomColors<slot>.json`, so it needs "Enable Customization" like everything else on that page. The Customize menu writes a `Preview` entry consulted before the record, so scrolling redresses immediately without touching the record; `EndPreview` on close, and a reset dress when customization was left off so the preview does not linger. Packs register at boot from `config.json` only and load synchronously on first wear (`SpineFolderLoader.Build` at the game's own follower asset scale, read from `WorshipperData.Instance.SkeletonData.skeletonDataAsset.scale` — attachment geometry is scaled at parse time, so a donor loaded at a different scale would be the wrong size). That means ONE hitch per pack the first time anyone scrolls to it in the Customize menu (tens of ms for a stripped-rig export, a visible stall for a full-rig one), then free; followers already wearing a pack load it inside the scene load. Preloading at boot with a per-pack `preload: false` opt-out is the recommended fix if that hitch matters; async-with-caption like player spines is the other. Not mirrored over MP: the record is host-side only.
- **Never `SetSlotsToSetupPose` on a follower.** It resets every slot's colour along with its attachment, and `SetFollowerCostume` tints the follower (skin colour set, clothing tint) BEFORE the postfix runs — the first version did exactly that and every dressed follower went white. The game has several other paths that adjust the outfit after the fact as well. `Dress` collects the slot indexes it wrote (`touched`, from its own overlays and the re-added game layers) and re-resolves only those slots' attachments by name, current attachment name first, setup name second, leaving colour alone.
- **Load errors are silent unless asked for.** `SkeletonDataAsset.CreateRuntimeInstance(.., initialize: true)` parses with `GetSkeletonData(quiet: true)`, so a pack that fails to parse leaves nothing in the log; `EnsureLoaded` asks again with `quiet: false` purely to get the Spine error printed, and its own message distinguishes "files missing" from "did not parse". Two parse failures found this way: (1) **an atlas without a blank first line has no pages** — spine-unity's runtime atlas loader takes the line AFTER an empty line as a page name; Spine's exporter always writes that blank line, a hand-written atlas will not, and the symptom is `Materials not set for atlas asset` then `Error reading attachment`. (2) **`spine38.py` read varints as unbounded Python ints** while the runtime accumulates into a 32-bit int, so a draw-order offset of −12 came out as 4294967284; the editor tolerated it, the runtime threw `Index was outside the bounds of the array` reading `idle-up`. `varint` now wraps to signed 32-bit and `validate` checks every draw-order offset lands inside the slot list. The previously shipped `followerSkel/stripped/Follower.json` had the bad offsets and was regenerated; a `.spine` made from the old one should be re-imported.
- **Portraits are SkeletonGraphics, and a SkeletonGraphic draws with ONE texture** (this spine-unity has no allowMultipleCanvasRenderers; see `PlayerUiSkin`). Its texture is the first submesh's, so with a cross-atlas sleeve early in draw order the WHOLE follower was drawn through the pack's 256² page — body a wash of the robe's blue, face gone, hat correct, exactly the screenshot. Fix is the mechanism the player already has: `SkeletonGraphic.Initialize` is patched to record graphic↔Skeleton in a `ConditionalWeakTable`, and `Dress` attaches a `PlayerUiMirror` to the owning graphic when it wrote anything (offscreen SkeletonAnimation sharing the same Skeleton, camera → RenderTexture → RawImage over the alpha-zero graphic) and detaches it when the follower shown wears nothing. This covers every UI that calls `SetFollowerCostume` on a graphic's skeleton — info box, tailor items, indoctrination outfit items — without patching each.
- **The wardrobe obeys both toggles.** The saved look applies only while the record's `CustomFollowerCostume` is on, and the menu's `Preview` is written as null unless Enable Customization AND Follower Costume Override are on (`SyncWardrobePreview`, re-run from both toggle handlers), so flipping either toggle undresses immediately. Selection is remembered while off and comes back when the override is re-enabled.
- **`tools/spinecheck`** (dotnet console, gitignored with the rest of tools/) parses a folder's `.json` + `.atlas` with the game's `spine-unity.dll` via `AtlasAttachmentLoader` and a no-op `TextureLoader` — the exact checks the game runs minus textures — so a pack's runtime errors show up in seconds instead of after a game restart. `dotnet run --project tools/spinecheck -- <folder>`. It found both failures above.
- **Multiplayer: the look never left the host (contract 4).** COTL MP Steam hands the guest the host's whole `DataManager` (`SaveHandoff`), so `FollowerInfo`s with the host's ids exist on both machines, and `FollowerSync` re-sends a whole `FollowerInfo` when its hash changes — but `CustomColors<slot>.json` is CultTweaker's own file, never shipped (`ContentManifest` lists `FollowerSkins` but not `FollowerSpines` either), and the guest's `SaveAndLoad.Load` postfix never runs for a world received over the wire, so the guest was applying ITS OWN slot's records to the host's follower ids. The API now carries the record as an opaque string (`FollowerLook`/`ApplyFollowerLook`, JSON of `CustomFollowerColor`; names only, so no id remap), `MirrorFollowerLooks(on)` stashes the guest's own dictionary (`CustomColorHelper.SetMirroring`) and blocks `Save`/`Load` while on, and `ReloadFollowerWardrobe` rescans packs that arrive late. `Redress` goes through the static 5-arg `FollowerBrain.SetFollowerCostume(skeleton, info, hooded, forceUpdate)`, which calls the patched 12-arg one, so the colour postfix and `Dress` both run. The MP side was built the same day (protocol 29): `MsgId.FollowerLook = 118`, `Sync/FollowerLookSync.cs` diffing the look string itself every 1.5 s, an initial full pass off a new `SaveHandoff.OnWorldReceived`, `MirrorFollowerLooks(true)` armed unconditionally on the guest whenever contract ≥ 4 is probed (reflectively, via `GetRawConstantValue`, because a referenced const bakes into their assembly), `FollowerSpines` in `ContentManifest.LiveFolders` with `ReloadFollowerWardrobe()` on arrival, and `FollowerSync.Look()` widened to include hat/clothing so a vanilla hat change alone redresses too. Untested by a two-player run as of 2026-09-14. Their log lines: host "follower looks: sent N to <peer>", guest "follower looks: showing the host's; your own are set aside until the session ends".
- **Scope that was there before.** `FollowerSpines/` was scanned by a stale `LoadAllFollowerSpines` that built an asset, registered it with COTL_API and then did nothing with it; that, `FollowerSpineConfig` (old shape), and the unused `CustomFollowerSpineSkin` record were removed. `FollowerSpineConfig` now names the pack's hats and clothes.

### Follower skin bake: what merging atlases would and would not buy (SpineLoaderHelper/FollowerSpineLoader.cs, FollowerSkinCache.cs)

- **One complete page per VARIANT, base body included.** `ComposeSkin` copies every attachment of the base animal and `GetRepackedSkin` packs the whole thing, so thirteen Cat variants are thirteen pages each carrying the full Cat body art. The wardrobe is the opposite design: nothing is repacked, custom parts keep their own material, and base art stays on the game's page.
- **Merging variants into one atlas would NOT speed up a cold bake.** The cost is region extraction from the 8192² BC7 page (whole-page decompress per `CopyTexture`, ~121ms a region), memoised in `AtlasUtilities.CachedRegionTextures` so the FIRST variant on a base animal pays for all of them and later ones are dictionary hits; `PackTextures` itself is ~5ms once warm. Merging removes no extraction and makes the pack one bigger atomic call.
- **On a cached boot merging would help only by sharing base regions.** Each cached page duplicates the animal body, so decode time is ~variants × body pixels; a merged page storing every base region once would decode far fewer. Plain concatenation of the existing pages decodes the same pixels and gains nothing, and either merge makes invalidation worse (one edited variant = the whole page rebuilt and redecoded). The contained wins, if ever wanted, are cache-side: decode the cached png off the main thread, or share base regions across variants of one animal.
- **Baking from a mod-shipped stripped page (the 363 Cat cut-outs packed into one readable RGBA32 2048²) WOULD make a cold Cat bake ~100ms:** remap each base attachment onto the shipped page's same-named region with `SetRegion` (what `FollowerSkinCache.TryApply` already does) and every repack source is uncompressed, so extraction is a memcpy. Not done for the shipped mod, and the reasons are not technical: it redistributes Massive Monster's follower art (the current design extracts from the player's own install on purpose); it needs one page per base animal; it goes stale when a game update redraws a follower (the cache hashes the game version for exactly that); and the disk cache already makes it a first-boot-only saving. The one place it pays is the skin editor's authoring loop — a local, opt-in source an author already has from working on the rig would make the first-edit extraction stall (the thing `Prebake` hides) disappear. Untried.
...
- **Broom transmog:** the brooms are the player skeleton's own `Mops/1`…`Mops/10` skins, each holding one attachment — `Tools/Mop` on slot `TOOLS` — pointing at a different `Tools/Mop_<n>` region. So a broom is the fleece mechanism with a one-entry override list (BroomOverrideSlots), and ApplyOverrideSlots is the body both share. Vanilla picks one by chore level in `PlayerFarming.SetSkin` (`FindSkin("Mops/" + Clamp(ChoreXPLevel + 1, 1, 10))`, added to the freshly built PlayerSkin), which is why DressBroom must run from the SetSkin postfix like DressFleece and not once at pick time — every skin rebuild puts the earned broom back. No cross-spine lift the way fleeces have: a custom player spine carries its own Mops skins (checked: every skin in PlayerSkins/ has all ten), so ResolveBroomSkin is a plain FindSkin on the wearer's own data, and a spine missing the skin keeps its own broom rather than borrowing one whose regions aren't in its atlas. The list is DISCOVERED (CollectBrooms scans Data.Skins for the `Mops/` prefix) rather than hardcoded to ten, sorted by the trailing number because a string sort puts `Mops/10` before `Mops/9`. Defaults off — unlike the fleece there is no prior CultTweaker behaviour to preserve, and on would silently override a chore level the player earned.
- **Donated brooms (config `brooms`) are the fleece donor scheme with one slot.** `BroomDonorEntries` reads the REGISTRY, not the loaded `BroomSpines` dictionary, so a donated broom is listed in the picker before its spine has ever been loaded — reading the loaded dict would mean nobody could pick the thing whose picking triggers the load. Names reuse the fleece form `CultTweaker_<spine>_<skin>` with the same 3-part capped split, so one parser serves both and a skin name may contain underscores. Registration sits OUTSIDE the `IsFleece` / wearable branch in `Register`: donating brooms is orthogonal to being wearable, and putting it in either arm would have forced authors to choose. **Slot indexes are per skeleton**, so a donor skin must be read with the DONOR's numbering — `ApplyOverrideSlots` takes an optional `sourceData` and resolves the read index by scanning `sourceData.Slots` for the name (the same trick `CustomWeapons` uses, because `SkeletonData` has no `FindSlotIndex`); passing the wearer's index against a donor whose slot order differs silently returns a different attachment. `OnlyAttachmentOn` is the forgiving path for hand-built donor skins that did not name their attachment `Tools/Mop`: one attachment on the slot is unambiguous, two or more is refused rather than guessed. Cross-atlas works for free because the attachment object copied in carries its own region and material from the donor's atlas — the same reason fleeces cross atlases. `CurrentBroomName` is persisted alongside the index for the same reason the fleece name is: at boot the index means nothing until the rotation is built, but the name says which spine to load eagerly.
- **The portrait sweeps once when a broom is chosen.** The `TOOLS` slot is empty in every resting pose, so a broom that has just been picked is invisible — the F7 panel would report a change nobody could see. Turning the transmog on, or picking a different broom, therefore runs `PlayerPreview.PlayOnce` with `cleaning`, the animation the game itself plays over a poop. `PlayOnce` reads the current track back and queues it again behind the one-shot so the portrait returns to rest, and when the current track already IS the one-shot (the same broom picked twice) it queues an idle instead — queueing nothing would freeze the portrait on the last frame of a sweep. It resolves through `First(exact: true, ...)`, the version WITHOUT the "anything with idle in the name" sweep, which is right for a resting pose and wrong for one particular action.
- **The sweep waits for the look to LAND, it is not played when asked for.** A donated broom whose spine is still preparing makes `ApplyBroom` defer and come back by callback seconds later, so sweeping at pick time shows the OLD broom — the same trap `LookChanged` was created for on the fleece side. So `ShowOffBroom` only ARMS the sweep and `PlayerDock.SweepIfWanted`, called from `OnLookChanged` after the re-dress, spends it. That inverts the caller order from the obvious one: arm BEFORE `ApplyBroom`, because a broom already to hand announces its look synchronously from inside that call, and arming afterwards would find the signal already gone. Two guards keep a stray sweep from firing later: a 30-second window, so a load that never finishes cannot leave one primed to go off on an unrelated fleece pick; and the arm survives a `PlayOnce` that found no portrait (a spine swap in flight), so it goes on the next look that lands instead of being silently eaten.
...
- **The field is 38px, flat, and outlined in white at 0.14** (it was 44px with a gold ring), and the labelled variant draws its name muted and its value white on one line. The popup rows are flat tints rather than ribbon buttons, and the row for the current value carries a 3px accent bar and white text -- a long list also opens scrolled to it rather than at the top. What did *not* change is the mechanism: the overlay still parents to `CanvasRoot`, still registers as a blocker, still closes through `NotifyDropdownOpened`/`CloseTransientUi`, and `PositionList` still converts with a null camera because every host is a screen-space-overlay canvas.
...
- **A custom structure's row shows its name, not its registry key.** The name a mod's structure is filed under is `CULT_TWEAKER_STRUCTURE_<NAME>` — that prefix is 23 characters before anything distinguishing starts, and in a panel this narrow it was the whole row. `StructureTool.ShortName` takes it off and sentence-cases what is left (the registry upper-cased whatever the author typed, so the original case is already gone). It is a label only: `TryGetPlacedName` and everything written to a file keep the full internal name, because that is the string that resolves back to the right structure on the next launch.
- **It is not a tool.** It is a panel `RuntimeMapEditor` owns, created in `CreateUi` BEFORE the `_ownChrome` sweep so F6/screen tools hide it with the rest, and ticked from `Update` (guarded like the tool update). It no longer sizes itself: it lives in the sidebar's lower slot, reports `WantedHeight`, and is given a height by `EditorSidebar.Layout`. Order matters in `LateUpdate` - the sidebar allocates first, then the tree lays out, because the tree only draws the rows its viewport can show. `OnToolChanged` hides it for `LevelTool` and `DungeonBuilderTool`, which own the screen.
...
- **The fold is the sidebar's, not the tree's.** The panel carries no header and no plate of its own; `EditorSidebar` raises `LayersToggled` and the tree mirrors it into `SetCollapsed`, which calls `SetLayersOpen` back. Both sides early-out when the state already matches, so the round trip stops after one hop. The old mutual exclusion with the shortcut hints is gone with the hints themselves: they are chips in the bottom bar now and never contest the sidebar.
- **Filtering narrows rows, not the room.** `Matches` is an ordinal case-insensitive contains; `Filtered` keeps every item of a section whose *name* matches, otherwise only the items that match themselves. While a filter is set, every section and folder is forced open - a match hidden inside a fold would look like no match at all - and the header summary reads "n of m". The filter row is a `MapEditorSearchRow` in its own fixed slot above the scroll view, never inside the virtualised column, whose rows are placed by hand at their index.
...
### The whiteboard tool (Tools/WhiteboardTool.cs; hooks in SelectTool.cs, RoomSnapshot.cs, BlueprintLoader.cs, BaseDelta.cs, Net/*)

- **What it is.** Freehand planning marks drawn in world space over the room: `CTWhiteboardStroke` (id, colour, width, points) plus a `LineRenderer` per stroke, all under one `CultTweaker_Whiteboard` root inside `SceneRefs.ContentRoot`, so a blueprint load's `ClearEditorContent` takes them with everything else the editor placed. The panel is exactly the requested layout: Show whiteboard, Draw, Draw size, a swatch row of nine fixed colours, Erase, Erase size. Draw and Erase are one hand (`SetMode`): turning one on puts the other down, and picking a colour switches to Draw. Preferences are static so they survive tool switches and editor sessions.
- **Not selectable, not content.** `SelectTool.IsPickIgnored` refuses anything under a `CTWhiteboardStroke`, which also keeps strokes out of the layer tree (it lists `AllSelectableRoots`); `RoomSnapshot.ShouldSkip` skips the root so the scenery sweep never records it as a prop. Strokes ARE in the blueprint (`CTNodeBlueprint.Whiteboard`, `MapStrokeData` with flat x,y pairs) through `IMapDataContributor`, so they save, load (`BlueprintLoader` after the trigger phase, `BaseDelta.ApplyContent` in the base) and sync like everything else. They render only while the editor is open, the whiteboard is shown and F6 has not hidden the chrome (`RefreshVisibility`, called from `EnterEditorMode`, `ExitEditorMode`, `ToggleChromeHidden`, `ShowChrome`); in play they are invisible.
- **Strokes and undo.** One drag is one stroke: points are appended at `max(0.03, width*0.3)` spacing, a click without a drag becomes a two-point dot, a scribble past 4000 points rolls into a new stroke. Release pushes `whiteboard stroke` whose undo is `RemoveStroke(id)` (false when a peer already erased it, so it is skipped). Erasing walks the segment the mouse just travelled and cuts every point within reach; the first surviving run keeps the stroke's id, the others become new strokes, runs under two points vanish, a fully erased stroke is removed. One erase drag records each touched stroke's record once (`_eraseBefore`) and the ids it created (`_eraseCreated`); its undo removes the created ones and `ApplyRecord`s the originals back.
- **Sync is the document, as designed.** `stroke:<id>` entries (`EditorDocument.Stroke`, order 19, `StrokeKind` always in place: `ApplyRecord` / `RemoveStroke`). The tool implements `IMapEditorLivePreview`: while drawing the current stroke, while erasing every touched and created stroke, and `EditorLive` sends their records reliably at the shape interval so the peer watches the line grow; deletions from an erase land with the diff on release. Random ids from `EditorIds.Of`, so two players drawing at once never collide. Nothing on the network side was touched to add the tool, which was the point of the design.
- **Clear all.** A quiet button under Erase size: finishes any gesture in progress, records every stroke's `MapStrokeData`, removes them all and pushes one `whiteboard clear` whose undo `ApplyRecord`s them back (same ids, so a peer's later diffs still match). The peer sees it as plain deletions in the next diff; nothing special on the network.
- **Mouse on the stroke plane.** Strokes and the ring cursor sit at `StrokeZ` (-1), not on the floor. The game camera is tilted, so a point on z 0 and the same x,y on z -1 land on different screen rows; projecting the mouse with `MouseWorld()` (floor plane) drew every stroke a few pixels above the cursor. The tool uses `RuntimeMapEditor.ScreenToWorldAtDepth(mouse, StrokeZ)` instead, and the ring shares that z (sorting order puts it above strokes). DEAD END: nudging the ring to `StrokeZ - 0.1` only moved it further from the cursor, for the same reason.
- **Swatch selection ring.** UI children always draw over their parent, so a "ring" plate added as a stretched child of the colour covered the colour with near-opaque white (the first bug report). The ring is the swatch's own 30 px `Image` (toggled with `enabled`, kept as a raycast non-target) and the colour is a child inset 3 px, so the ring shows as a border. The colour child is the button's raycast target.
- **Icon.** `Assets/EditorIcons/Whiteboard.png` is a generated placeholder (board, three squiggles, a marker); replace it with real art when there is some.

### Multiplayer editing (MapEditor/Net/*; hooks in RuntimeMapEditor.cs, BlueprintLoader.cs, BaseDelta.cs, RoomSnapshot.cs, the tools)

- **The network never sees a tool.** The room is treated as a document: the same `CTNodeBlueprint` every tool already writes into for a save (plus the base's journals in the base), taken apart into entries `"kind:id" -> canonical JSON` by `EditorDocument.Collect`. After an edit the room is re-collected, diffed against the last document the peer saw (`EditorOps`), and only the changed entries travel. The receiver (`EditorApply`) writes each entry back with an `EntryKind` that wraps the spawn and remove code the tools and the blueprint loader already have. A new tool is synchronised by putting its objects in the blueprint through `IMapDataContributor` and registering one `EntryKind`; nothing on the network side changes.
- **CultTweaker owns no transport.** `EditorNet` is the seam: a multiplayer mod installs an `IEditorNetLink` (send bytes, post a notice, ship files) and calls `EditorNet.Receive` with bytes coming back. Channel numbers and payload formats (`EditorWire`, JSON with floats rounded to 3 dp; snapshots GZipped) are CultTweaker's own and opaque to the link. With no link installed every call is a no-op and the editor is unchanged. The contract is versioned (`ContractVersion`): add members on a new interface, never change the existing ones - the other mod probes for the type before installing.
- **Every object now has an id.** `CTEditorId` is a tag component on the selection root, like `CTEditorGroup`; records carry it as `Id` (and props their island's id as `ParentIslandId`), the loader adopts it on spawn, so ids survive save and load. Objects the editor creates get random ids that travel with their creation record; objects both machines already have (vanilla scenery, kept authored pieces, the base's own pieces) get a deterministic id from what they are and where they stand (`EditorIds.Seed`, FNV-1a), so two machines that generated the same room name its pieces the same way without talking. Triggers keep their own `Trigger<N>` id; a guest's are suffixed `g` so two players placing at once never mint the same one. Doors are named by direction. A recycled pooled object sheds its tag (postfix on `ObjectPool.Recycle`) so the pool cannot hand an id to something else.
- **Who owns what.** The host's room is the real one. A guest says hello when its room content is ready or its editor opens; the host answers with a full snapshot only if its room has drifted from the files (an editor open, or unsaved edits). The guest reconciles (`EditorApply.Reconcile`): entries whose id it has are diffed; entries it lacks are first matched against local objects with no counterpart in the snapshot (same kind, same key, same place within 0.05 - `EntryKind.Matches`) and adopt the host's id, so a seeded room is not torn down; whatever is left over locally is removed. A "Resync with <peer>" button on the shortcut card repeats the exchange.
- **Deletes win, and undo stays local.** Deleted keys are tombstoned for 5 s on both sides so a late change to a deleted object is dropped. Remote changes are applied with `MapEditorHistory.Suspended` set so a peer's work never enters the local undo stack; a local undo whose target the peer removed returns false and is skipped, and the status line says how many stale steps were dropped (`Undo(out description, out skipped)`). Undoing a move on something the peer has since moved wins over the peer's position - last write wins, the diff ships it.
- **Echo suppression is by re-collection, not by remembering bytes.** After a batch is applied, `EditorOps.AbsorbApplied` re-collects the room and stores exactly those keys' collected form in the baseline; anything a local edit changed in the meantime still goes out, and the applied entries never bounce back even where the receiver's canonical form differs from the sender's (a profile name it lacks, a scale read back through `lossyScale`).
- **Live drags are a separate stream.** Tools that drag implement `IMapEditorLivePreview` (Select, Trigger, Door, Shape); while a gesture is in progress `EditorLive` sends changed transforms at 20 Hz unreliable and shape/trigger records at 10 Hz reliable, and the document diff waits until the mouse is up, then sends the authoritative record. The receiver only writes transforms (`ApplyRemoteShape(bake:false)` for shapes); collision is baked when the reliable record lands.
- **The unreliable streams carry a sequence number and are eased, not snapped.** `PresenceMsg.Seq` and `LiveMsg.Seq` count per sender; the receiver drops anything not newer than the last it took (`EditorPresence.Newer`, wraparound-safe on a `ushort`), because Steam's unreliable messages arrive in any order and a late one would step the cursor or the dragged object back a frame. Records (shapes, triggers) are reliable and ordered and always apply. The peer's cursor is drawn for `CursorGraceSeconds` (0.3 s) after the sender reports no cursor, since the sender drops it every time its own mouse crosses a panel - without the grace the cursor flickered through the whole editor-open. Peer-dragged transforms go into `EditorLive._glides` and `Smooth()` moves the object toward the latest one every frame with the cursor's easing (`1 - exp(-14 dt)`); the first message of a drag snaps so nothing glides in from across the room. When the authoritative record lands, `EditorApply` brackets the upsert with `BeginAuthoritative`/`EndAuthoritative`: the record's transform becomes the glide's destination and the object is put back where the glide had it, so the release finishes smoothly instead of jumping; the drag lock lifts at that moment too. `EditorOps.AbsorbApplied` takes the record itself as baseline for a still-gliding object, so the last frames of glide never echo back as an edit. Remote lighting records apply with a 0.25 s fade (`LightingKind.RemoteFadeSeconds`); the local slider keeps zero fade.
- **Presence is the lock.** `EditorPresence` sends cursor, tool and selected ids at 10 Hz while editing (reliably on change). The peer's selection is drawn amber (`#F2A03D`: object tint, outline boxes through `MapEditorGizmos`, cursor ring and name, a dot after the layer-tree row); the local one stays cyan. `LockedByPeer` refuses selection, drag and delete of anything the peer has selected or is dragging, with "In use by <name>." On a race where both pick the same object, the local cyan keeps the tint and the peer gets only the outline.
- **Only the host writes files.** A guest's Save (button, Ctrl+S, Save & close) sends a `SaveRequest` with the chosen name; the host runs its own `WriteMapRoutine` under that name and then calls `Link.ShipFiles` with the written relative paths (`CustomBaseMaps/base_slot<N>.json`, or `CustomNodeBlueprints/<name>.json` + `.png`, plus `CustomLevelBlueprints/<name>.json` for a hub) and a `Saved` message that marks the guest's editor saved. The guest's disk is only ever written by the link.
- **A player who is not editing still receives.** In the base a hidden host is created the same way `BaseDelta.ApplyRoutine` creates one, `RuntimeMapEditor.ContextOverride` makes the tools behave as in the base editor for the duration, `BaseDelta.SetApplying(true)` lets placed structures become buildings, and the editor's `Map` is pointed at `BaseDelta.Content` so tool contributions land in the same blueprint the host edits. `EditorDocument.EffectiveContext` decides base-vs-room by the scene, not by whether a base session is open - that is also what the room key uses, so a guest standing in the base without the editor open still accepts base messages. The base's own journals (`Removed`, `Moved`, `EditedShapes`, `MovedStructures`) travel as entries too (`vremoved`, `vmoved`, `vshape`, `smoved`) and `BaseDelta.Apply*Entry` writes them into the receiver's in-memory file so both copies say the same thing.
- **The time hold does not wait for a peer.** `EditorNet.PausesTime` is `LocalEditorOpen || (Enabled && RemoteEditorOpen)`. The multiplayer mod's time-scale loop runs as soon as a session exists (hosting, nobody joined yet) and rewrites the editor's 0 to 1 every LateUpdate unless something asks for a hold; gating the local half on a connected peer let enemies placed with the Enemy tool walk and attack while the host was alone in a lobby. The MP side must gate its query on its bridge being installed, not on a connected peer, for the same reason.
- **Levels follow the host (`Channel.Level`, `LevelPlayback.OnPeerLevel/ArmFromPeer`).** The guest cannot start a level, but its own `BiomeGenerator` still has to build the host's floor, and two things about that floor are not in the multiplayer mod's seed message: an authored layout is built from the level file by the `CreateRandomWalk` prefix, and the blueprint each slot plays was drawn at random by `Resolve` on the host. So `LevelPlayback.Start/StartForMapNode/Stop` call `EditorNet.LevelChanged`, which ships `{Active, LevelName, Rooms}` (also on link-up and on every Hello, for a guest that arrives mid-level). The guest loads the level by name from its own files (content sync already put it there), keeps it armed, and `DungeonPatches.BiomeGenerator_OnEnable` calls `ArmFromPeer` first thing: it adopts the level and the resolved rooms, sets `CTLevelDungeon.Instance.Level` and `EnteringCustomDungeon`, and from there every hook runs as on the host. `FromPeer` marks that state; the host's `Active=false` stops it. **DEAD END that was the bug:** the host's `LevelLayout.Build` sets `OverrideRandomWalk = true` after filling `Rooms` itself, the multiplayer seed message copies that flag with an empty `OverrideRooms`, and vanilla `CreateRandomWalk` then builds zero rooms and never calls `MMTransition.ResumePlay` - the guest sat on the loading screen forever. The `CreateRandomWalk` prefix now clears that combination when no level is active, so a guest without the level file at least gets a floor. The multiplayer mod should still remap the seed's `DungeonLocation` through its COTL_API id table; a custom `FollowerLocation` is not the same int on both machines.
- **Enemies in a session are two populations, and the one that lingers is the other mod's.** The multiplayer mod mirrors every host enemy on the guest (`EnemySync`: an `EnemySpawn` per host `UnitObject.OnEnable`, a puppet spawned after a 5 s grace unless a local unit of the same name within 3 units adopts the id) and treats every Team2 unit on the guest as a puppet (`StopAllCoroutines`, no pathing). Our document also spawns the same enemies on the guest, so each placed enemy can exist twice there; the copy that never receives a host snapshot is hidden after 4 s (`never matched the host; hiding it`). The mirror is only ever removed by an `EnemyDied` message, sent from a `UnitObject.OnDie` postfix on the host. The editor destroys enemies WITHOUT a death all the time - delete, undo, clear, a reload, a respawning upsert - and then the guest's mirror stays behind: alive, driven by nothing, an animated body doing nothing (the user's report of 2026-09-06). The fix belongs to the multiplayer mod: send an "enemy gone" when a registered host unit is disabled or destroyed without dying, and better, key enemy ids on `EditorNet.ObjectId(go)` (contract 2) so a placed enemy is matched exactly on both machines instead of by name and rounded position.
- **DEAD END fixed on our side: a missing baseline used to mean respawn.** `EntryKind.CanApplyInPlace(previous, json)` returns false when `previous` is null, and a receiver that is not editing (or was just reconciled) has no baseline, so every upsert for an existing enemy destroyed and respawned it - on the host that is a unit destroyed without death, which is exactly the zombie above. `EditorApply.ApplyBatch` now takes the scene's current record (one `Collect` per batch, lazily) as `previous` when the baseline has none.
- **What the editor gates when connected.** `EditorNet.WhyNotChangeWorld` (set by the link) is consulted by `HubSession.Begin`, `LevelPlayback.Start/StartForMapNode`, `DungeonBuilderTool.EnterDungeon`, F5 and the editor's reset room; the link answers for a guest with the multiplayer mod's own "the host decides" wording. While the peer's chat box has the keyboard (`ExternalTyping`) the editor takes no hotkeys, camera keys or tool input; while the chat is on screen (`ExternalOverlayVisible`) the bottom bar's chip row hides, since the chat rests on the bar.
- **Reading the logs.** Both mods write to `BepInEx/LogOutput.log`; the bracket names the side (`[Info   :CultTweaker]` vs `[Info   :COTL MP Steam]`). Every line of ours starts with `EditorNet:` and talks about entries, kinds, ids, collect/apply/reconcile; the other mod's start with `[MP] editor:` and talk about channels, bytes, chunks and files. The boundary is `EditorNet.Receive`: every exception inside it is caught and logged with its stack as `EditorNet: message on channel N failed`, so a stack trace after the other mod's "received" line is ours. Config `MapEditor / NetVerbose` (`Plugin.EditorNetVerbose`) logs every batch sent and applied with its keys and every reconcile count; off by default. The end of a session logs one summary line with changes sent, applied, snapshots and failures - a "sent" count on one machine that does not match "applied" on the other is the first thing to check. Live and presence streams log only their first message.
- **Single-machine self-test.** With `MapEditor / NetVerbose` on, the shortcut card gains a "Sync self-test" button (`EditorApply.SelfTest`): it collects the room, encodes and decodes the snapshot, then re-applies every entry to itself with no previous record (the respawn path for props, structures, enemies, npcs, podiums; the in-place path for the rest) and collects again. One log line reports collect time, snapshot size, wire differences, apply failures and entries that read back differently, then one line per differing key. Run it in a heavy room before a two-machine test: it exercises every `EntryKind` and the canonical form without a peer. Local undo entries for respawned objects go stale, as they would after a peer's change.
- **Seeds for stacked vanilla shapes.** Vanilla rooms put several `Sprite shape 0 (N)` floors of one profile at the origin, so profile plus position collided and the second got a `~2` suffix whose owner depended on `FindObjectsOfType` order. The shape seed now includes the prefab's object name and the point count. The `id ... moved from ... to ...` line on a blueprint load is routine (the parked vanilla piece hands its id to the loaded copy) and is logged only under NetVerbose.
- **DEAD END considered: replaying the undo history to the peer.** `MapEditorHistory` entries are reverse closures only; there is no forward form, and most drags mutate transforms before any commit point. Shipping per-tool messages was rejected for the same reason the document exists: every new tool would need its own message, its own applier and its own version bump on the network side.
- **Known limits.** Hub co-editing needs both players inside the hub, which the guest cannot enter until the multiplayer mod replays hub entry. The base and custom dungeon levels work end to end. `ClearTool` destroys locked objects too (a bulk action, by design). The world map editor, Level tool and Dungeon Builder are not wired; they are pure JSON documents and would ride the same mechanism as one entry each.
